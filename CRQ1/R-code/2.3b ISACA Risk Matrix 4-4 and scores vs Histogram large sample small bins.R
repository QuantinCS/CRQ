...
LowestSig = min(sp) * min(si)
HighestSig = max(sp) * max(si)
...
dsr$risk = "Sig"
dsr$lowest = LowestSig
dsr$highest = HighestSig
dsr$Confidence <- ifelse(dsr$x >= dsr$lowest & dsr$x <= dsr$highest, "In", "Out")
dsr$RealRank <- ifelse(dsr$Confidence == "In", "6 TrueHigh", "5 FalseHigh")
...
risk_thresholds <- c(0, HighestLow, LowestMed, HighestMed, LowestSig, HighestSig, LowestHig, HighestHig)
